...
def get_channel_json():
    ...
    playlists = sorted(
        playlists,
        key=lambda x: [k.lower() for k in PLAYLISTS.keys()].index(
            x.get("title").lower()
        ),
    )
    ...
    for item in items[:max]:
        if item.get("_type") == "url" and item.get("id"):
            if any([item.get("id") == v.get("video_id") for v in result]):
                print(
                    f"    --- skipping duplicate {item.get('id')} :: {item.get('title')}"
                )
                continue
...
def get_video_details(video_id):
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                f"DEBUG: yt-dlp returned non-zero exit code for {video_id}: {result.stderr}",
                file=sys.stderr,
            )
            # print(result.stderr)
            return None
